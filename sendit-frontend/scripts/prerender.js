...
const staticPages = [
  {
    ...
    body: [
      ...
      "For normal file sharing, you can select up to 20 files with a maximum of 100 MB per file. The sender chooses an expiry of up to 24 hours, then shares the generated four-digit code through a private channel. A single-file share downloads after code validation; a multi-file share presents the files individually.",
      "SendIt also supports temporary code snippets. Paste code, choose one of 14 language choices, set an expiry, and let the recipient open it in a read-only syntax-highlighted viewer. Code is not executed by the service, and recipients can copy anything they receive.",
      "This is a transfer tool rather than cloud storage or backup. Access is blocked when a share expires, cleanup runs separately, and operational or history metadata may remain. Use short expiry windows, keep your own copy of important files, and never upload secrets you do not need to share.",
      "A common use is moving a document between a laptop and a phone without signing into a new storage account. Upload the document on one device, open the receive page on the other, and enter the code before the selected expiry. The same flow works for a project handoff, a draft for review, or a small set of images that should not become a permanent shared folder.",
      "For developers, a temporary snippet can be easier to discuss than a screenshot or a badly formatted chat paste. The language choice preserves readable indentation and syntax colors, while the expiry keeps an old debugging example from remaining active forever. Treat the access code as a bearer credential: send it only to the people who need it.",
      "SendIt does not promise anonymity, end-to-end encryption, or control over a copy after a recipient downloads it. HTTPS protects data in transit, stored file content is encrypted by the backend, and server-side expiry checks stop normal access after the share closes. Read the Security, Privacy Policy, FAQ, and Sharing Guide pages before using the service for sensitive work.",
      "The receive process is intentionally separate from the send process. A sender can explain the code and expiry in a private message, while the recipient only needs the matching receive page and an active code. If a code is mistyped or expired, the correct recovery is to contact the sender and create a new share; SendIt does not recover expired content as a backup service.",
      "Before uploading, check the filename, remove unnecessary personal information, and confirm that you have permission to share the material. Do not use a temporary transfer as a substitute for a managed workspace when a team needs version history, ongoing permissions, or reliable long-term access. These limits are part of the product design, not hidden exceptions.",
    ],
    links: [
      ["/send", "Send a file"],
